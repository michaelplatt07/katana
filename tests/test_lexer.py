...
class TestLexerMacroKeyword:
    ...
    def test_macro_successfully_lexes(self):
        program = Program(["main() {\n", "MACRO myMacro {\n", "3 + 4;\n", "}\n", "print(myMacro);\n", "}\n"])
        token_list = get_main_tokens() + [
            Token(MACRO_KEYWORD_TOKEN_TYPE, 0, 1, "MACRO", 4),
            Token(MACRO_NAME_TOKEN_TYPE, 6, 1, "myMacro", 0),
            Token(LEFT_CURL_BRACE_TOKEN_TYPE, 14, 1, "{", 3),
            Token(NUM_TOKEN_TYPE, 0, 2, "3", 0),
            Token(PLUS_TOKEN_TYPE, 2, 2, "+", 1),
            Token(NUM_TOKEN_TYPE, 4, 2, "4", 0),
            Token(EOL_TOKEN_TYPE, 5, 2, ";", 0),
            Token(RIGHT_CURL_BRACE_TOKEN_TYPE, 0, 3, "}", 3),
